...
public class MedicalEquipmentDao {
	...
	public List<Equip_Records> getMedicalEquipmentList(String serialNo, String assetNo){
		...
		if(null != serialNo && !serialNo.isEmpty()){
			SQL = SQL + " WHERE a.SERIAL_NO = ? AND a.IsActive = ?";
			params.add(serialNo);
			params.add("1");
		}else if(null != assetNo && !assetNo.isEmpty()){
			SQL = SQL + " WHERE a.int_ast_no = ? AND a.IsActive = ?";
			params.add(assetNo);
			params.add("1");
		}else{
			SQL = SQL + " WHERE a.IsActive = ?";
			params.add("1");
		}
	...
	public List<Equip_Records> getMedicalEquipmentListFromRecords(String serialNo, String assetNo){
		String SQL;
		SQL = "SELECT * FROM pems_database.equip_records WHERE SERIAL_NO = ? AND IsActive = ?";
	...
	public int deleteEquipMentRecord(String equipId) {
		String SQL = "UPDATE pems_database.equip_records SET  IsActive = ? WHERE equip_record_id = ?";
